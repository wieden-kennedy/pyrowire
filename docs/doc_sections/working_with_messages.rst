...
To help work with messages, pyrowire relies on *handlers* which are basically just methods assigned to a specific message topic.
...
The two criteria for creating a handler are:
   1. the pyrowire.handler annotation must take one kwarg, *topic*, and must be set to the topic for which the handler is to work.
